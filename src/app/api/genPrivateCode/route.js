...
function generatePrivateCode(length = 6) {
  ...
  async function privateCodeExists(privateCode) {
    try {
      const sql = 'SELECT COUNT(*) AS count FROM Orders WHERE private_key = ?';
      const result = await query(sql, [privateCode]);
      console.log('Checking existence of private code:', privateCode, 'Result:', result);
  ...
  async function generateUniquePrivateCode(length = 6) {
    ...
    do {
      ...
      if (attempts > maxAttempts) {
        throw new Error('Unable to generate a unique private code after multiple attempts');
      }
